...
- **Shipped:** M0 (Scaffold), M1 (Load + Render), M2 (Playback), M3 (Command layer + Undo/Redo), M4 (Global edits), **M5 (Overlay projector + Selection + bar-highlight playhead)**, and **M6 (Chords — own pills + dropdown chord editor)** are all merged to `main` (M4 [PR #6](https://github.com/nndrch/MusiPad/pull/6) `784cd81`; M5 [PR #7](https://github.com/nndrch/MusiPad/pull/7) `91207eb`; M6 [PR #9](https://github.com/nndrch/MusiPad/pull/9) `ca9160d`). **M7 (Sections, Annotations, Download + Print)** is complete and in review (this PR, on `feat/m7-slashes-sections-annotations-download`).
...
- **In flight:** none — **M7 is complete** (Sections + Annotations shipped on top of the earlier Download + Print, this PR). The two **note-level** M7 items were **extracted to post-MVP** (2026-06-08): per-bar slash toggle → **P11** (built, on `post-mvp/slash-toggle`), note respell + A3 menu → **P12** — both need note editing (P9/P10) to be worth it, and our chart notes are slash placeholders. **Next: M8 — Meter / time-signature editing**, then **M9 — Polish**.
...
| M7  | Sections, Annotations, Download + Print                                 | ✅ Done    | (this PR)                                               | (this PR)             |
...
- **Scope (final):** Download + Print (shipped earlier in M7, `d30ffe8`) **+ Sections + Annotations** (this PR). The two **note-level** items were de-scoped to post-MVP — slash toggle → **P11** (built, on `post-mvp/slash-toggle`), note respell + A3 menu → **P12** (both 2026-06-08).
