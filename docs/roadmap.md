...
- **Shipped (merged to `main`):** **M0 → M8** — Scaffold, Load + Render, Playback, Command layer + Undo/Redo, Global edits, Overlay projector + Selection + bar-highlight playhead, Chords (dropdown), Sections + Annotations + Download + Print, and **M8 — Meter / time-signature editing** (M8 [PR #16](https://github.com/nndrch/MusiPad/pull/16) `b8418e1`; M7 [PR #15](https://github.com/nndrch/MusiPad/pull/15) `8235d8d`; M6 [PR #9](https://github.com/nndrch/MusiPad/pull/9) `ca9160d`).
- **Next direction — chord-chart simplification + A4 pages (post-M8 feedback, 2026-06-15).** Extended-team feedback reframed the tool as a friendly **chord-chart editor** (later a lead-sheet editor), explicitly **not** a Sibelius/MuseScore competitor. Two new milestones land ahead of the old Polish pass:
  - **M9 — Simplified chord-chart rendering:** render **only** bars + chord symbols + a per-beat slash grid (N = meter numerator) + a **double barline at each section start**; hide the clef, key glyph, and melody. Every bar is **normalized to the slash grid in the real DOM on load** (source notes replaced → a chord attaches to any beat); OSMD draws the staff while we overlay our **own** minimal slashes (centered via staff geometry). Invariant #2 vs the normalized-on-load baseline. Playback + Download disabled. PRD §6.5 / §9 M9.
...
- **In flight:** the **chord-chart pivot specs** (this branch, `docs/chord-chart-pivot-specs`) — PRD / roadmap / post-MVP updates only, **no code yet**. **Next build: M9 — Simplified chord-chart rendering** (PRD §6.5), then **M10 — A4 page layout + toggle + print** (§6.6), then **M11 — Drag-reorder chords**, then **M12 — Polish**.
...
| M9  | Simplified chord-chart rendering (slashes + chords only)                | ⏳ Planned | —                                                       | —                     |
| M10 | A4 page layout + view toggle + print                                    | ⏳ Planned | —                                                       | —                     |
| M11 | Drag-to-reorder chords (snap to slashes; from P17)                       | ⏳ Planned | —                                                       | —                     |
...
### M9–M12 ⏳ Planned — chord-chart simplification, A4 pages, chord drag
...
- **M9 — Simplified chord-chart rendering:** only bars + chord symbols + a per-beat slash grid (N = meter numerator) + a **double barline at each section start**; hide clef, key glyph, melody. Every bar is **normalized to the slash grid in the real DOM on load** (source notes replaced, so a chord attaches to any beat); OSMD draws the staff, we overlay our **own** minimal slashes centered via the staff-line geometry; section dividers are our own overlay too. Invariant #2 vs the normalized-on-load baseline. Playback + Download disabled for this milestone. PRD §6.5 / §9 M9.
- **M10 — A4 page layout + view toggle + print:** paginated A4 pages (4 bars/row, breaks between systems) + a page-layout ↔ fullscreen toggle; clean multi-page Print. Reuses the P4 `pageFormat:'A4_P'` foundation; PDF download stays P4. PRD §6.6 / §9 M10.
- **M11 — Drag-to-reorder chords:** drag a chord pill, snapping to the nearest slash (within/across bars); a `moveChord` command over the M6 chord commands. Promoted from P17 (2026-06-15). PRD §9 M11.
