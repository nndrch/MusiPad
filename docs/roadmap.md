...
- **Next direction — chord-chart simplification + A4 pages (post-M8 feedback, 2026-06-15).** Extended-team feedback reframed the tool as a friendly **chord-chart editor** (later a lead-sheet editor), explicitly **not** a Sibelius/MuseScore competitor. Two new milestones land ahead of the old Polish pass:
  - **M9 — Simplified chord-chart rendering:** render **only** bars + chord symbols + a per-beat slash grid (N = meter numerator) + a **double barline at each section start** (persisted on download); hide the clef, key glyph, and written melody (preserved in the DOM, never shown). The slash grid lives in the OSMD render clone (`buildRenderDoc`) — the real DOM is untouched by it (Invariant #2). PRD §6.5 / §9 M9.
  ...
  - **M11 — Polish** (renumbered from the old M9): toasts, empty/error states, keyboard shortcuts, design audit.
...
| M11 | Polish (renumbered from M9)                                             | ⏳ Planned | —                                                       | —                     |
...
### M9–M11 ⏳ Planned — chord-chart simplification + A4 pages
...
- **M9 — Simplified chord-chart rendering:** only bars + chord symbols + a per-beat slash grid (N = meter numerator) + a **double barline at each section start** (persisted on download); hide clef, key glyph, and the written melody (preserved in the DOM, never shown) — the slash grid lives in the OSMD render clone (real DOM untouched by it; Invariant #2). PRD §6.5 / §9 M9.
...
- **M11 — Polish:** toasts, empty/error states, keyboard shortcuts, design audit (renumbered from the old M9).
