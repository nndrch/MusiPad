...
- **Dropped from the MVP (was M6b):** the structured root/quality/bass picker, enharmonic toggle, and chord-root respell were prototyped then cut as too complex (2026-06-08) — the shipped editor is the **dropdown** combobox; typing covers every spelling/quality/slash. Parked in [`post-mvp-improvements.md`](./post-mvp-improvements.md) P8. Non-`root` harmonies (`numeral`/`function`) remain out of scope — preserved, never corrupted.
