...
**In review — M13 (Audio-synced review playback).** Load the paired recording (`_stabilised.wav` + its `.bpm`) and **play it in sync** with the chart: the current bar highlights and auto-scrolls to follow (a toggle), a **metronome** keeps place, **clicking a bar** seeks + play/pauses, and **editing pauses the audio at the playhead** so you correct chords/sections/notes against what you hear, then resume. Currently in external review on the PR preview ([#22](https://github.com/nndrch/MusiPad/pull/22)).

**Then — M14–M15.** A **root × quality** double-dropdown chord picker (M14), then a final **Polish** pass (M15). Specs in [PRD §9](./docs/musicxml-editor-prd.md) and [`docs/roadmap.md`](./docs/roadmap.md).
