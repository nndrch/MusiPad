...
6. **Re-weigh the post-MVP backlog every 3 milestones.** After each third milestone ships (M3, M6, …), review `[docs/post-mvp-improvements.md](./docs/post-mvp-improvements.md)` and **promote the low-hanging items that now fit an upcoming milestone**: weigh effort × value × fit, move the winners into the PRD §9 milestone scope + `[docs/roadmap.md](./docs/roadmap.md)`, and trim them from the backlog (leave a "Promoted to a milestone" note). Keep genuinely heavy items deferred — don't bloat milestones. Surface the weighing to the human and let them pick what to schedule.
